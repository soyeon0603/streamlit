...
df_melted = pd.melt(df, id_vars=['date'], var_name='team', value_name='sales')
...
chart = alt.Chart(df_melted, title='일별 팀 매출 비교').mark_line().encode(x='date', y='sales', color='team', strokeDash='team').properties(width=650, height=350)
...
chart = alt.Chart(df_melted, title='일별 팀 매출 비교').mark_bar().encode(x='date', y='sales', color='team')
text = alt.Chart(df_melted).mark_text(dx=0, dy=0, color='black').encode(x='date', y='sales', detail='team', text=alt.Text('sales:Q'))
st.altair_chart(chart+text, use_container_width=True)


chart = alt.Chart(df_melted, title='일별 팀 매출 비교').mark_bar().encode(x='date', y='sales', color='team')
text = alt.Chart(df_melted).mark_text(dx=-1, dy=3, color='black').encode(x='date', y='sales', detail='team', text=alt.Text('sales:Q'))
st.altair_chart(chart+text, use_container_width=True)
...
fig = px.bar(medal, x='nation', y=['gold', 'silver','bronze'],
             text_auto=True, title='올림픽 양궁 금메달 현황')
st.plotly_chart(fig)
